...
#
#
# We may read in the original parameter file as well, in case the information becomes useful here:
...
# Plot pitch distributions for all speakers (separating male and female speakers with line type):
png("fig/density_utt_all.png", width=600, height=400, units="px", bg="white", pointsize=14)
#postscript("fig/density_utt_all.eps", horizontal=FALSE, onefile=FALSE, paper="special", height=5, width=5, pointsize=12)
par(mar=c(3,4,3,1))
par(mfrow=c(1,1))
f.Hz <- pretty(range(semitonesToHertz(pitch$PitchST),na.rm=TRUE))
for (spk in 1:length(speakers)){
...
#postscript("fig/density_Hz_F3.eps", horizontal=FALSE, onefile=FALSE, paper="special", height=5, width=5, pointsize=12)
par(mar=c(3,4,3,1))
par(mfrow=c(1,1))
for (spk in 1:length(speakers)){
	speaker = speakers[spk]
...
#
# 
# First, try to bootstrap the mode, median, mean and standard deviation for each speaker:
#
#
...
steps = ntest / step
#
#-----  Now bootstrap the MODE for all speakers in one figure:
bootstrap.mode <- data.frame(rbind(c(1:steps*step),c(1:steps*step*1.5)))
# Maximum number of pitch samples
...
#for (spk in 1:3){
	speaker = speakers[spk]
	# This is the leftmost column for this speaker in the bootstrap table:
	spk.row = 1 + (spk-1) * 5
	bootstrap.mode[c(spk.row:(spk.row+4)),] = NA
	par(mar=c(4,4,3,1))
	par(mfrow=c(1,1))
	...
	for (test in 1:steps) {
		...
		if (nrow(data) > lowerlimit) { 
			for (draw in 1:5) {
				...
				if (n > 5) {
					points(n,mode,pch="o",cex=0.5,col="grey")
					bootstrap.mode[row,test] = mode
					}
				}
				row = row + 1
				} 
...
bsmodes.mean = sapply(bootstrap.mode[,c(1:ncol(bootstrap.mode))],mean,na.rm=TRUE)
lines(c(1:nrow(bootstrap.mode)*50), bsmodes.mean,lwd = 2)
bsmodes.sd = sapply(bootstrap.mode[,c(1:ncol(bootstrap.mode))],sd,na.rm=TRUE)
lines(c(1:nrow(bootstrap.mode)*50), bsmodes.mean + bsmodes.sd,lwd = 1)
lines(c(1:nrow(bootstrap.mode)*50), bsmodes.mean - bsmodes.sd,lwd = 1)
graphics.off()
#
#-----  Now bootstrap the MEAN for all speakers in one figure:
bootstrap.mean <- data.frame(rbind(c(1:steps*step),c(1:steps*step*1.5)))
...
#for (spk in 1:5){
	...
	# This is the leftmost column for this speaker in the bootstrap table:
	spk.row = 1 + (spk-1) * 5
	bootstrap.mean[c(spk.row:(spk.row+4)),] = NA
	par(mar=c(4,4,3,1))
	par(mfrow=c(1,1))
	data = subset(pitch,pitch$Speaker==speaker)
	totalmean= mean(data$PitchST,na.rm=TRUE)
	# Draw up to ntest consecutive samples randomly and calculate the summary statistics in each case:
	for (test in 1:steps) {
		n = test * step
		data = subset(pitch,pitch$Speaker==speaker)
		row = spk.row
		for (draw in 1:5) {
			...
			if (n > 5) {
				points(n,mean,pch="o",cex=0.5,col="grey")
				bootstrap.mean[row,test] = mean
				}
			}
			row = row + 1
		}
...
bsmeans.mean = sapply(bootstrap.mean[,c(1:ncol(bootstrap.mean))],mean,na.rm=TRUE)
lines(c(1:nrow(bootstrap.mean)*50),bsmeans.mean,lwd = 2)
bsmeans.sd = sapply(bootstrap.mean[,c(1:ncol(bootstrap.mean))],sd,na.rm=TRUE)
lines(c(1:nrow(bootstrap.mean)*50),bsmeans.mean+bsmeans.sd,lwd = 1)
lines(c(1:nrow(bootstrap.mean)*50),bsmeans.mean-bsmeans.sd,lwd = 1)
graphics.off()

bootstrap.mean.means <- summary(bootstrap.mean[,c(3:202)])

bootstrap.tab = data.frame(cbind(speakers,0,0,0,0,0,0))
rownames(bootstrap.tab) = bootstrap.tab[,1]
colnames(bootstrap.tab) = c("Speaker","Mode1000","Mode3000","Mode6000","Mean1000","Mean3000","Mean6000")
bootstrap.tab$Mode1000 <- factor(bootstrap.tab$Mode1000)
bootstrap.tab$Mode3000 <- factor(bootstrap.tab$Mode3000)
bootstrap.tab$Mode6000 <- factor(bootstrap.tab$Mode6000)
bootstrap.tab$Mode1000 <- factor(bootstrap.tab$Mode1000)
bootstrap.tab$Mode3000 <- factor(bootstrap.tab$Mode3000)
bootstrap.tab$Mode6000 <- factor(bootstrap.tab$Mode6000)

#-----  Plot density curves of mode-referred pitch at specific stages of the bootstrapping process:
...
#-----  Plot density curves of mean-referred pitch at specific stages of the bootstrapping process:
# For the article, we plot figures at 
# test=20 -> 1000 samples
# test=60 -> 3000 samples
# test=120 -> 6000 samples
